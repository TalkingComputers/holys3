...
- S3 now uses the official AWS SDK credential, endpoint, signing, and operation implementations while retaining s3grep's adaptive concurrency, retries, hedging, range coalescing, and bounded body storage.
- The minimum supported Rust version is now 1.94.1, required by the current AWS SDK.

### Removed

- The internal `s3grep-sigv4` crate and custom IAM Identity Center credential exchange.
