use s3grep_core::Strategy;
use s3grep_index::{search_collect, update_index, SegmentedReader, SourceIdentity, UpdateOptions};
use s3grep_s3::{is_index_key, FetchConfig, S3BlobStore, S3Client, S3Corpus};

#[test]
fn live_s3_index_search_roundtrip() -> anyhow::Result<()> {
    let bucket = match std::env::var("S3GREP_TEST_BUCKET") {
        Ok(bucket) => bucket,
        Err(_) => {
            eprintln!("skipping: set S3GREP_TEST_BUCKET to run");
            return Ok(());
        }
    };
