...
use s3grep_core::{BlobStore, MatchOptions, Strategy};
use s3grep_index::{
...
#[command(
    ...
    long_about = "s3grep PATTERN TARGET searches a prebuilt index.\n\
        TARGET is s3://bucket[/prefix].\n\
        To search for a pattern named like the `index` subcommand,\n\
        use -e: `s3grep -e index s3://bucket`."
)]
struct Cli {
    #[command(subcommand)]
    cmd: Option<Cmd>,
...
fn split_pattern_target(args: Vec<String>, regexp: Vec<String>) -> Result<(Vec<String>, String)> {
    ...
    let [pattern, target] = <[String; 2]>::try_from(args)
        .map_err(|_| anyhow::anyhow!("usage: s3grep PATTERN TARGET"))?;
...
fn build_s3(
    ...
    let (progress, bar) = if show_progress {
        let (sender, receiver) = s3grep_core::ProgressSender::channel();
...
fn build_s3_inner(
    ...
    let listing = list_user_objects(src, index, progress.as_ref())?
        ...
        .collect::<Vec<_>>();
    if let Some(progress) = &progress {
        progress.emit(s3grep_core::ProgressEvent::ListingComplete {
...
fn build_cache_dir(endpoint: Option<&str>, bucket: &str, prefix: &str) -> Result<PathBuf> {
    let mut path = s3grep_core::cache_home()?;
    path.push("s3grep");
    let scope = format!("{}\0{bucket}\0{prefix}", endpoint.unwrap_or(""));
    path.push(format!(
        "{bucket}-{:016x}",
        s3grep_core::hash_cache_scope(scope.as_bytes())
    ));
    Ok(path)
}
...
mod tests {
    ...
    fn clap_parses_rg_style_invocations() {
        ...
        let cli = Cli::try_parse_from(["s3grep", "index", "s3://b", "--purge-deleted"]).unwrap();
        assert!(matches!(
            cli.cmd,
            Some(Cmd::Index {
                purge_deleted: true,
        ...
        let cli = Cli::try_parse_from(["s3grep", "-e", "index", "s3://b"]).unwrap();
        assert!(cli.cmd.is_none());
        assert_eq!(cli.search.regexp, vec!["index"]);
        // last case flag wins
        let cli = Cli::try_parse_from(["s3grep", "-i", "-s", "p", "t"]).unwrap();
        assert!(cli.search.case_sensitive && !cli.search.ignore_case);
        // --json conflicts with -c
        assert!(Cli::try_parse_from(["s3grep", "--json", "-c", "p", "t"]).is_err());
    }
    ...
    fn clap_parses_independent_index_locations() {
        ...
        assert_eq!(
            ...
            Some("s3://search-index/s3grep/logs")
        );
