...
use rayon::prelude::*;
#[cfg(test)]
use s3grep_core::pack_trigram_grams;
use s3grep_core::{
...
/// bodies); an index built by an older s3grep must error, not silently
...
pub trait IndexReader: DocFetcher {
    ...
    ) -> Result<Vec<s3grep_core::DocAddress>>;
    fn visit_candidates(
        &self,
        q: &Query,
        key_prefix: Option<&str>,
        batch_size: usize,
        visit: &mut dyn FnMut(Vec<s3grep_core::DocAddress>) -> Result<bool>,
    ) -> Result<()> {
...
fn read_local_body(
    ...
    let stale = || {
        anyhow::Error::new(s3grep_core::StaleSource {
            key: key.to_owned(),
...
fn read_local_group(
    ...
    s3grep_core::decode_requested_body(&group.key, &group.requests, body, consume)
}
...
impl DocFetcher for LocalFetcher {
    fn fetch_each(
        &self,
        documents: &[s3grep_core::DocAddress],
        consume: &mut dyn FnMut(usize, DocumentBody) -> Result<()>,
    ) -> Result<()> {
...
mod tests {
    use super::*;
    use s3grep_core::testutil::MemCorpus;
    use s3grep_core::BlobStore as _;
    use s3grep_core::{LineEvent, LineKind, LocalBlobStore, MatchOptions, SubMatch};
    ...
    fn splits_sparse_postings_into_bounded_runs() {
        let text = (0..16_384usize)
            ...
            .collect::<Vec<_>>();
        let expected = s3grep_core::sparse_grams_all_bytes(&text);
        let runs = write_posting_runs(
        ...
        let mut expected_hashes: Vec<u64> = expected
            .iter()
            .map(|gram| s3grep_core::hash_ngram(gram))
            .collect();
        ...
        for gram in expected {
            let packed = lookup(s3grep_core::hash_ngram(&gram)).expect("indexed sparse gram");
    ...
    fn candidate_superset_then_verify() {
        ...
        for strategy in [Strategy::Trigram, Strategy::Sparse] {
            ...
            let cands = r
                .candidate_docs(&s3grep_query::plan("world", r.strategy()).unwrap(), None)
                .unwrap();
    ...
    fn local_large_sources_stay_file_backed() {
        ...
        let document = s3grep_core::DocAddress {
    ...
    fn local_fetch_rejects_stale_source_version() {
        ...
        let error = LocalFetcher::new(1)
            ...
            .unwrap_err();
        assert!(error.is::<s3grep_core::StaleSource>(), "{error:#}");
    }
