...
//! ```
//!
//! `s3grep index` becomes a diff: list the bucket, compare (key, etag)
...
use s3grep_core::{BlobStore, Corpus, DocAddress, IndexAddress, ProgressSender, Strategy};
...
pub fn update_index(
    ...
    if let Some(progress) = progress {
        progress.emit(s3grep_core::ProgressEvent::DiffComputed {
            to_add: added as u64,
    ...
    anyhow::ensure!(
        ...
        "another s3grep index run updated this index concurrently; rerun to pick up its result"
    );
...
impl SegmentedReader {
    ...
    fn load(
        ...
        let (bytes, root_version) = store
            .get_versioned("segments.bin")
            .context("reading segments.bin")?
            .context("no index found — run `s3grep index` first")?;
        let list = parse_segment_list(&bytes)
            .context("index is not usable as-is; run `s3grep index` to rebuild")?;
    ...
    fn read_candidate_batches(
        ...
        for (segment_id, segment) in self.segments.iter().enumerate() {
            ...
            let lookup = |gram: &[u8]| match &remote_values {
                Some(values) => Ok(values.get(&s3grep_core::hash_ngram(gram)).copied()),
...
/// `S3GREP_SPARSE_REMOTE_MIN` overrides the byte threshold (testing and
/// forced-mode verification); a malformed value fails loudly.
fn sparse_remote_terms_min() -> Result<u64> {
    parse_remote_terms_min(std::env::var("S3GREP_SPARSE_REMOTE_MIN").ok().as_deref())
}
...
fn parse_remote_terms_min(configured: Option<&str>) -> Result<u64> {
    match configured {
        ...
        Some(value) => value
            .parse()
            .with_context(|| format!("S3GREP_SPARSE_REMOTE_MIN is not a byte count: {value:?}")),
    }
}
...
impl s3grep_core::DecodeSink for SampleWindow {
    fn begin(&mut self, _: &s3grep_core::LogicalDocumentMeta) -> Result<()> {
        Ok(())
    }
...
fn detect_strategy(
    ...
    if !picks.is_empty() {
        ...
        for (idx, (key, _, _)) in picks.iter().enumerate() {
            ...
            if let Err(error) =
                s3grep_core::decode_source(key, bytes, s3grep_core::DECODE_LIMITS, &mut sample)
            {
                if !error.is::<SampleWindowFull>() {
                    continue;
                }
            }
            match s3grep_core::is_prose_like(&sample.window) {
                Some(true) => {
...
/// Byte cap for the on-disk pack/postings range cache. `S3GREP_CACHE_MAX`
...
fn range_cache_max() -> Result<u64> {
    ...
    match std::env::var("S3GREP_CACHE_MAX") {
        Ok(value) => value
            .trim()
            .parse::<u64>()
            .with_context(|| format!("S3GREP_CACHE_MAX is not a byte count: {value:?}")),
...
impl s3grep_core::DocFetcher for SegmentedReader {
    fn fetch_each(
        &self,
        documents: &[DocAddress],
        consume: &mut dyn FnMut(usize, s3grep_core::DocumentBody) -> Result<()>,
    ) -> Result<()> {
...
mod tests {
    ...
    fn remote_terms_threshold_rejects_malformed_configuration() {
        ...
        assert!(
            error.to_string().contains("S3GREP_SPARSE_REMOTE_MIN"),
            "{error:#}"
        );
    }
    ...
    fn unreadable_root_advises_reindex_only_on_the_search_path() {
        ...
        assert!(
            !format!("{parse_error:#}").contains("run `s3grep index`"),
        ...
        let store = s3grep_core::LocalBlobStore::new(dir.path());
        ...
        assert!(
            format!("{open_error:#}").contains("run `s3grep index` to rebuild"),
            "{open_error:#}"
        );
    }
    ...
    fn index_update_rejects_source_change_without_rebuild() {
        ...
        let store = s3grep_core::LocalBlobStore::new(store_dir.path());
    ...
    fn segment_root_references_uploaded_content_packs() {
        ...
        update_index(
            ...
            UpdateOptions::default(),
            &|_| {
                Ok(Box::new(s3grep_core::testutil::MemCorpus::new(
                    vec!["a.txt".to_owned()],
                    vec![b"alpha".to_vec()],
                )))
            },
    ...
    fn segment_tables_reject_mismatched_key_bounds() {
        let tables = SegmentTables {
            sources: vec![SourceEntry {
                ...
                version: "v1".into(),
                encoded_size: 1,
                encoding: s3grep_core::SourceEncoding::Raw,
                first_doc: 0,
                doc_count: 1,
                failed: false,
                retry: false,
    ...
    fn segment_cap_stops_before_later_archive_failure() {
        ...
        let body = s3grep_core::testutil::encode::zip(&[
            ("a.log", b"a"),
            ("b.log", b"b"),
            ("c.log", b"c"),
