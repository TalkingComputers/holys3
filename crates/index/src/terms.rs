    ...
    pub(crate) fn get(&self, gram: &[u8]) -> Result<Option<u64>> {
        match self {
            ...
            Self::Sparse {
                ...
                let hash = s3grep_core::hash_ngram(gram);
...
mod tests {
    ...
    fn sparse_visit_round_trips_through_a_new_builder() {
        ...
        let mut entries: Vec<(u64, u64)> = grams
            .iter()
            .enumerate()
            .map(|(value, gram)| (s3grep_core::hash_ngram(gram), value as u64))
            .collect();
    ...
    // s3grep-core) share one dictionary entry: lookups for either gram see
    // the merged value — a candidate superset, never a lost document.
    #[test]
    fn colliding_grams_share_a_merged_entry() {
        let a: &[u8] = b"czopbaaa";
        let b: &[u8] = b"plo ba";
        let hash = s3grep_core::hash_ngram(a);
        assert_eq!(hash, s3grep_core::hash_ngram(b), "fixture must collide");
