...
fn update_index_reports_progress_events() -> Result<()> {
    use s3grep_core::{ProgressEvent, ProgressSender};
...
fn progress_receiver_drop_does_not_affect_indexing() -> Result<()> {
    use s3grep_core::ProgressSender;
    let mut bucket = Bucket::default();
    bucket.put("a.log", b"alpha needle line\n");
    let store_dir = tempfile::tempdir()?;
    let cache_dir = tempfile::tempdir()?;
...
fn archive_members_follow_source_lifecycle() -> Result<()> {
    ...
    let query = s3grep_query::plan("needle", Strategy::Trigram)?;
...
struct BudgetedPut<'a> {
    inner: Box<dyn s3grep_core::StreamingPut + 'a>,
    budget: &'a AtomicUsize,
}

impl s3grep_core::StreamingPut for BudgetedPut<'_> {
...
impl BlobStore for FailingStreamStore {
    ...
    fn put_streaming<'a>(&'a self, name: &str) -> Result<Box<dyn s3grep_core::StreamingPut + 'a>> {
...
fn colliding_gram_hashes_keep_results_exact() -> Result<()> {
    ...
    let collides = s3grep_core::iterate_sparse_grams(doc_a.as_bytes()).any(|gram_a| {
        s3grep_core::iterate_sparse_grams(doc_b.as_bytes()).any(|gram_b| {
            gram_a != gram_b && s3grep_core::hash_ngram(gram_a) == s3grep_core::hash_ngram(gram_b)
        })
    });
