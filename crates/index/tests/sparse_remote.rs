...
//! `S3GREP_SPARSE_REMOTE_MIN`, which is process-global state, so they live in
//! their own integration binary and run as one sequential test.

use anyhow::Result;
use s3grep_core::{testutil::MemCorpus, BlobStore, LocalBlobStore, Strategy};
use s3grep_index::{search_collect, update_index, SegmentedReader, SourceIdentity, UpdateOptions};
...
fn remote_readers_cache_the_index_tail_and_match_cached_mode() -> Result<()> {
    let _lock = env_lock();
    let _remote_min = EnvGuard::new("S3GREP_SPARSE_REMOTE_MIN");
    let _cache_max = EnvGuard::new("S3GREP_CACHE_MAX");
    ...
    let listing: Vec<(String, String, u64)> = objects
        ...
        .map(|(key, body)| {
            (
                key.clone(),
                format!("{:016x}", s3grep_core::hash_ngram(body)),
                body.len() as u64,
            )
        })
        .collect();
    ...
    std::env::set_var("S3GREP_SPARSE_REMOTE_MIN", "1");
    ...
    std::env::set_var("S3GREP_SPARSE_REMOTE_MIN", u64::MAX.to_string());
    let cached_mode = open()?;
    assert_eq!(search(&cached_mode, "quick brown fox")?, remote_hits);
    std::env::remove_var("S3GREP_SPARSE_REMOTE_MIN");
    Ok(())
}
...
fn range_cache_evicts_to_its_cap_and_stays_correct() -> Result<()> {
    ...
    std::env::remove_var("S3GREP_SPARSE_REMOTE_MIN");
    std::env::remove_var("S3GREP_CACHE_MAX");
    update_index(
    ...
    std::env::set_var("S3GREP_CACHE_MAX", "1");
    ...
    std::env::set_var("S3GREP_CACHE_MAX", "0");
...
fn repeat_queries_serve_ranges_from_the_local_cache() -> Result<()> {
    ...
    let _cache_max = EnvGuard::new("S3GREP_CACHE_MAX");
    std::env::remove_var("S3GREP_SPARSE_REMOTE_MIN");
    std::env::set_var("S3GREP_CACHE_MAX", (4u64 << 30).to_string());
