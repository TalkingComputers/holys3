...
`s3grep-bench` is the unpublished deterministic performance harness. Its local
backend exercises the index engine only; the `s3grep` product CLI remains
S3-only.
...
`xbench/` and index data under `xbench/.s3grep/`.

```sh
AWS_PROFILE=your-profile \
S3GREP_BENCH_BUCKET=your-bucket \
S3GREP_BENCH_REGION=us-east-1 \
...
```sh
cargo run --locked --release -p s3grep-bench -- seed --seed 1 --objects 25000 --size 4096
cargo run --locked --release -p s3grep-bench -- upload --target dir
cargo run --locked --release -p s3grep-bench -- churn --cycles 30 --changes 250
```
