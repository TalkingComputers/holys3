...
use s3grep_core::{LocalBlobStore, Strategy};
use s3grep_index::{
    search_streaming, update_index, IndexReader, KeyScope, LocalCorpus, NullSink, SegmentedReader,
    SourceIdentity, UpdateOptions,
};
use s3grep_s3::{build_fetch_config, build_index_namespace, S3BlobStore, S3Client, S3Corpus};
...
fn run(scenarios_path: &Path, iterations: usize, warmup: usize, concurrency: usize) -> Result<()> {
    ...
    let has_bucket = read_optional_env("S3GREP_BENCH_BUCKET")?.is_some();
    let has_endpoint = read_optional_env("S3GREP_BENCH_ENDPOINT")?.is_some();
    let summary = match (has_bucket, has_endpoint) {
        (true, _) => run_s3(scenarios, &manifest, iterations, warmup, concurrency)?,
        (false, true) => anyhow::bail!("set S3GREP_BENCH_BUCKET with S3GREP_BENCH_ENDPOINT"),
...
fn measure_search(reader: &dyn IndexReader, pattern: &str) -> Result<SearchMeasurement> {
    ...
    let stats = search_streaming(
        ...
        pattern,
        KeyScope::default(),
        s3grep_core::MatchOptions::default(),
        &NullSink,
    )?;
...
fn read_s3_backend(concurrency: usize) -> Result<S3Backend> {
    let bucket = std::env::var("S3GREP_BENCH_BUCKET")?;
    let region = std::env::var("S3GREP_BENCH_REGION")?;
    let endpoint = read_optional_env("S3GREP_BENCH_ENDPOINT")?;
