...
**Tech Stack:** Rust 1.88, clap derive, ctrlc 3.5.2 with `termination`, serde/serde_json, std mpsc, existing s3grep index/store APIs, GitHub Actions, MinIO/local fixtures.
...
- Keep the S3 boundary in `s3grep-s3`; no network IO enters core/query/index.
...
**Interfaces:**
- Consumes: `s3grep_index::UpdateReport` and a caller closure `FnMut(bool) -> anyhow::Result<IndexResult>`.
- Produces:
...
pub(crate) struct IndexResult {
    pub report: s3grep_index::UpdateReport,
    pub location: String,
}
...
Run: `cargo test -p s3grep index::tests -- --nocapture`
...
- Produces the CLI contract `s3grep index TARGET [--watch --interval SECONDS] [--json]`.
...
Add the two exact tests to `cli.rs`; use `tempfile`, existing `s3grep()`, and `serde_json::Value`.

- [ ] **Step 2: Run tests to verify failure**

Run: `cargo test -p s3grep --test cli index_ -- --nocapture`
...
Create `watch.rs` with `#![cfg(unix)]`, spawn `env!("CARGO_BIN_EXE_s3grep")`, use a reader thread and `recv_timeout`, invoke the system `kill` command with `-TERM`, and perform the exact assertions in the contract.

- [ ] **Step 6: Run the real process test**

Run: `cargo test -p s3grep --test watch -- --nocapture`

Expected: 1 passed in roughly 1-3 seconds; child exits 0 and final search succeeds.

- [ ] **Step 7: Run all CLI tests**

Run: `cargo test -p s3grep`
...
Run: `cargo test -p s3grep-index --test segmented interrupted_root_swap -- --nocapture`
...
Run: `cargo test -p s3grep-index --test segmented`
...
Run: `cargo test -p s3grep-bench churn -- --nocapture`
...
```bash
cargo run --locked -p s3grep-bench -- seed --seed 1 --objects 100 --size 4096
cargo run --locked -p s3grep-bench -- upload --target dir
cargo run --locked -p s3grep-bench -- churn --cycles 3 --changes 10
```
...
**Interfaces:**
- Consumes: `s3grep-bench churn --cycles 10 --changes 250` and `crates/xbench/runs/churn.json` from Task 4.
...
```yaml
      - name: Profile incremental churn
        run: |
          /usr/bin/time -v -o bench-churn-rss.txt target/release/s3grep-bench churn --cycles 10 --changes 250 | tee bench-churn.txt
...
```bash
cargo build --locked --release -p s3grep -p s3grep-bench
target/release/s3grep-bench seed --seed 1 --objects 25000 --size 4096
target/release/s3grep-bench upload --target dir
/usr/bin/time -l target/release/s3grep-bench churn --cycles 10 --changes 250
```
...
1. In README quickstart, add `s3grep index ./logs --out s3grep.idxdir --watch --interval 30` and an S3 equivalent using a generic profile.
...
Run: `cargo run --locked -p s3grep -- index --help`
