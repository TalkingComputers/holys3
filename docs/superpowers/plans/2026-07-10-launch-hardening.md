...
**Goal:** Land, validate, publish, and verify s3grep 0.4.0 through one protected release path.
...
- Create `feat/s3grep-0.4.0` without discarding the dirty worktree.
...
- Push `feat/s3grep-0.4.0`, open a pull request, and wait for `ci-success`, `benchmarks-success`, and `security-success`.
...
- Verify `cargo install s3grep --version 0.4.0`, `cargo binstall s3grep`, SHA-256 checksum files, GitHub attestations, docs.rs, and `s3grep --version`.
