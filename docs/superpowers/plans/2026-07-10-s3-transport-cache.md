...
```bash
cargo +1.96.1 test --locked -p s3grep-s3 conditional_get
```
...
```bash
cargo +1.96.1 test --locked -p s3grep-sigv4 -p s3grep-s3 conditional_get
cargo +1.96.1 clippy --locked -p s3grep-s3 --all-targets -- -D warnings
```
...
```bash
cargo +1.96.1 test --locked -p s3grep-index grouped_sources_fetch_once
```
...
```bash
cargo +1.96.1 test --locked -p s3grep-index grouped_sources_fetch_once
cargo +1.96.1 test --locked -p s3grep-cli --test cli archive
```
...
```bash
cargo +1.96.1 test --locked -p s3grep-s3 ordered_ranges
```
...
```bash
cargo +1.96.1 test --locked -p s3grep-s3 ordered_ranges
cargo +1.96.1 test --locked -p s3grep-s3 --test minio range
```
...
```bash
cargo +1.96.1 test --locked -p s3grep-s3 cache
cargo +1.96.1 test --locked -p s3grep-cli --test cli object_cache
```
...
Count only source-body GETs and bytes; exclude index metadata and posting ranges using the reserved `.s3grep` namespace.
...
```bash
cargo +1.96.1 run --locked --release -p s3grep-xbench -- s3 --profile local
```
