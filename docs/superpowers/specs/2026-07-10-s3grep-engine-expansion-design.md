# s3grep Engine Expansion Design
...
Make s3grep materially faster and more scalable while preserving its exact-search contract, then expand the decoder from one searchable document per object to multiple logical documents per source object.
...
Every candidate carries the indexed source version. S3 `GetObject` sends `If-Match` with that ETag. HTTP 412 becomes a typed stale-index error that tells the user to rerun `s3grep index`; verification never silently runs against a different object version.
