...
[![CI](https://github.com/TalkingComputers/s3grep/actions/workflows/ci.yml/badge.svg)](https://github.com/TalkingComputers/s3grep/actions/workflows/ci.yml)
[![crates.io](https://img.shields.io/crates/v/s3grep.svg)](https://crates.io/crates/s3grep)
[![docs.rs](https://docs.rs/s3grep/badge.svg)](https://docs.rs/s3grep)

s3grep searches S3 buckets with regular expressions. It works like grep, but
...
```sh
s3grep index s3://my-logs/prod                # build the index, once
s3grep 'req-7f3e9a2c1b' s3://my-logs/prod     # then grep it
s3grep -i 'timeout' s3://my-logs -g '*.gz' -C2 --since 6h
```
...
s3grep pays the scan once, at index time. After that:
...
[GitHub release](https://github.com/TalkingComputers/s3grep/releases):

```sh
cargo binstall s3grep   # fetches the prebuilt binary for your platform
cargo install s3grep    # or build from source (Rust 1.94.1+)
```

Release archives include SHA-256 checksums and GitHub build-provenance
attestations. Verify one with
`gh attestation verify <archive> -R TalkingComputers/s3grep`.

## Usage

The shape is `s3grep PATTERN TARGET`, where TARGET is `s3://bucket[/prefix]`.
...
First build the index. It lives in the bucket, under `<prefix>/.s3grep/` by
default:

```sh
AWS_PROFILE=my-sso s3grep index s3://my-log-bucket/prod --region us-east-2
```

Then search:

```sh
s3grep 'level":"ERROR' s3://my-log-bucket/prod --region us-east-2
```
...
```sh
s3grep -i 'timeout' s3://my-logs -C2 -g '*.gz' -g '!debug/*'
s3grep -w -F 'foo(' s3://my-code-bucket -l
s3grep 'req-[0-9a-f]+' s3://my-log-bucket --json | jq .
```
...
name, use `-e`: `s3grep -e index s3://bucket`.

Searches can be scoped by key or by time:

```sh
s3grep 'ERROR' s3://my-logs --since 6h
s3grep 'ERROR' s3://my-logs --since 2026-06-09 --until 2026-06-10 --key-prefix prod/
```
...
```sh
s3grep index s3://my-log-bucket/prod --watch --interval 30
```
...
```sh
s3grep index s3://my-log-bucket/prod --index s3://my-search-index/prod
s3grep 'ERROR' s3://my-log-bucket/prod --index s3://my-search-index/prod
```
...
<picture>
  ...
  <img alt="How s3grep works: the index pipeline decodes objects and writes a trigram index plus content snapshot to S3; the search pipeline plans grams, fetches posting and pack ranges, and verifies with a real regex" src=".github/assets/how-it-works-light.svg">
</picture>
...
Use private buckets. The default index lives under `<source-prefix>/.s3grep/`,
...
as the source data. s3grep contacts the configured source and index S3
